...
\item Unless otherwise noted, an instruction cannot read a variable
unless the variable is in the
current function frame\label{CURRENT-FUNCTION-FRAME}.  This means
that the compile time location $\ell$ of the variable is in the range: \\
\hspace*{0.5in}{\tt [ mexstack::fp[mexstack::lexical\_level],
                    mexstack::stack\_length )} \\
It also means that the run time location $\ell'$ of the variable is in the
range: \\
\hspace*{0.5in}{\tt [ process->fp[process->level], process->length )} \\
and that: \\
\hspace*{0.5in}{\tt    mexstack::stack\_length - $\ell$
                    == process->length - $\ell'$ )}
...
{\em Variable-name} must name a variable in the current function
frame\pagnote{CURRENT-FUNCTION-FRAME}.
\end{indpar}
...
{\em Object-variable-name} must name a variable in the current function
frame\pagnote{CURRENT-FUNCTION-FRAME}.
\end{indpar}
...
\ttkey{GET} ~ {\em object-variable-name}
	      {\em new-variable-name}\QMARK{}
\begin{indpar}
At run-time this instruction's actions are as described in
Figure~\mexref{OBJECT-INSTRUCTIONS}, where {\em object-variable-name}
specifies the variable containing the object pointer (\TT{immedA} value).

At assembly time, pops one entry from the variable stack
and then pushes into the variable stack an
element with {\em new-variable-name} as its name.

{\em Object-variable-name}
must name a variable in the current function
frame\pagnote{CURRENT-FUNCTION-FRAME}.
\end{indpar}
...
\ttkey{SET} ~ {\em object-variable-name}
\begin{indpar}
At run-time this instruction's actions are as described in
Figure~\mexref{OBJECT-INSTRUCTIONS}, where {\em object-variable-name}
specifies the variable containing the object pointer (\TT{immedA} value).

At assembly time, pops the variable stack twice.
...
\ttkey{COPY} ~ {\em new-variable-name}\QMARK{}
\begin{indpar}
At run-time this instruction's actions are as described in
Figure~\mexref{OBJECT-INSTRUCTIONS}.

At assembly time, pops one element from the variable stack and then
pushes an element with {\em new-variable-name} as its name.
\end{indpar}

\ttkey{COPYI} ~ {\em list-element}\STAR{} ~ {\em new-variable-name}\QMARK{}
\begin{indpar}
At run-time this instruction's actions are as described in
Figure~\mexref{OBJECT-INSTRUCTIONS}, where \TT{immedD} is a read-only
list object of the form \TT{[~]} or
\TT{[} {\em list-element} \{ \TT{,} {\em list-element} \}\STAR{} \TT{]}.
{\em List-elements} may be numbers or strings.
...
\ttkey{NOP}/\ttkey{TRACE}/\ttkey{DUMP}/\ttkey{WARN}/\ttkey{ERROR} ~
...
The \TT{DUMP} instruction, which is used to dump the current
top values in the run-time stack, is exceptional in that it does not
push the {\em trace-variables} into the stack, but instead
uses the existing top of stack as {\em trace-variable} values,
and does \underline{not} pop the stack.  
\end{indpar}
...
\ttkey{TEST\_INSTRUCTION} ~
    ...
    \end{tabular}
\begin{indpar}
Fields after {\em op-code}
may be omitted from the end of the instruction.  Omitted
fields default to \TT{0}, except \TT{immedD} defaults to
\TT{min::MISSING()}.
...
If the smallest block containing a \TT{JMP\ldots} instruction is also
the smallest block containing the corresponding \TT{LABEL},
the {\tt mexstack::stack\_length} of the \TT{LABEL} must equal
the {\tt mexstack::stack\_length} just after the \TT{JMP\ldots}
if the latter is successful.
Note that for conditional jumps like \TT{JMPEQ} with \TT{immedB != 0},
the {\tt mexstack::\EOL stack\_\EOL length} after a successful jump is 2 less
that the {\tt mexstack::stack\_length} just before the \TT{JMPEQ},
even if the {\tt mexstack::stack\_length} just after the unsuccessful
jump is different (only 1 less).

If the smallest block containing a \TT{JMP\ldots} instruction is strictly
inside the smallest block containing the corresponding \TT{LABEL},
the {\tt mexstack::stack\_length} of the \TT{LABEL} must equal
the {\tt mexstack::stack\_length} just after the largest block
that contains the \TT{JMP\ldots} and is also
strictly inside the smallest block containing the \TT{LABEL}.
