...
\subsubsection{Compiling Instructions with MEX Stack}
\label{COMPILING-INSTRUCTIONS-WITH-MEX-STACK}

In order to output an instruction, a compiler using {\tt mexstack} does the
following:

\begin{enumerate}
\item Adjusts {\tt mexstack::stack\_length} to the value it will have
...
a compiler language statement or block.  At the end of assignment
statements, the variables created by the assignment statement are
pushed into the symbol table.  At the end of a block,
...
(e.g., \TT{MEXAS LABEL}\pagnote{LABEL} which calls
