...
\end{tabular}
\\[1ex]
\verb|typedef (* |\TTKEY{trace\_function}\verb|) ( mex::process p )|%
...
the process vector which is the top of the stack
(i.e., \TT{process[sp]}).  To push a value into you add
+1 to \TT{sp} and store the value in the element it now indexes.
To pop a value, you remove the value indexed by \TT{sp}, and then
add -1 to \TT{sp}.  The \TT{sp} member of a
...
\TT{ERROR}
    ...
    & the process terminates.  Otherwise \TT{immedA} is added to \TT{sp} \\
    & (popping the stack \TT{immedA} times).  See \itemref{ERROR-INSTRUCTION}.
\\\hline
\end{tabular}
\end{center}
\vspace*{-3ex}
...
{\tt mex::module mex::}\TTKEY{create\_module}
...
{\tt mex::process mex::}\TTKEY{create\_process} \verb|()|
\begin{indpar}
...
{\tt mex::process mex::}\TTKEY{init\_process}
    \verb|( mex::module m, mex::process p = min:NULL_STUB )|
\begin{indpar}
...
{\tt mex::process mex::}\TTKEY{init\_process}
    \verb|( mex::pc pc, mex::process p = min:NULL_STUB )|
\begin{indpar}
...
{\tt bool mex::}\TTKEY{run\_process}
    \verb|( mex::process p, min::uns32 limit = 0xFFFFFFFF )|
\begin{indpar}
Execute \TT{p} until it terminates or has executed \TT{limit}
instructions.  Returns \TT{true} on normal termination
and \TT{false} otherwise.  Normal termination is one of:
\begin{enumerate}
\item \TT{limit} instructions have been executed
\item \TT{pc} is \TT{\{min::NULL\_STUB,0\}}
\item \TT{pc} equals \TT{\{m,index\}} with \TT{index} == \TT{m->length}
\end{enumerate}

\end{indpar}

Note: You can use \TT{init\_process ( m )} followed by \TT{run\_process}
to run the code of \TT{m}.  You can then add to the end of \TT{m}
