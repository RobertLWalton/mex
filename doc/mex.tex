...
Each function block is assigned a (lexical) \key{level}.  A function block
not inside any other function block gets level 1.  Otherwise a function
block gets the level of its smallest containing function block plus 1.
Instructions and non-function blocks are given the level of their
smallest containing function block.  Instructions and non-function blocks
outside any function block are given level 0.
...
A downward function call is a call from a {\tt CALL\ldots} instruction
in a level $L$ function block to a level $L'=L+1$ function block.
The compiler maintains a varible stack that is in 1-1 correspondence
with the process stack as long as there are no upward calls.

A MEX compiler maintains a \key{variable stack} that approximates the process
stack.  If there are no upward calls 
