...
\verb|{                   | \\
\verb|mex:|\TTMKEY{TRACE}{in {\tt mex::trace\_flag}} & \tt = 1 <{}< 0 \\
\verb|mex:|\TTMKEY{TRACE\_PHRASE}{in {\tt mex::trace\_flag}} & \tt = 1 <{}< 1 \\
\verb|mex:|\TTMKEY{TRACE\_NOACTION}{in {\tt mex::trace\_flag}}
                                                     & \tt = 1 <{}< 3 \\
\verb|}                   | \\
\end{tabular}
...
The \TT{trace\_function}, \TT{excepts},
...
In normal mode, if an instruction's \TT{trace\_flags} value
is non-zero, the instruction is traced.
In optimized mode, no tracing is done.

When an instruction is traced and it's \TT{TRACE\_PHRASE} flag is on,
the instruction source lines are printed
...
Tracing an arithmetic instruction produces a message such as:
\begin{center}
\tt 1 + 2 ===> 3
\end{center}
...
or the \TT{mex::\EOL default\_EOL trace\_\EOL function}
...
will be no jump) and the instruction's \TT{TRACE\_NOACTION}
flag is off.

See \pagref{MEX::DEFAULT_TRACE_INSTRUCTION} for a description
of the \TT{mex::\EOL default\_\EOL trace\_\EOL instruction}.
