...
\TT{JMP}
    & Subtract \TT{immedA} to \TT{sp} (popping the stack \TT{immedA} times),
      then \\
    & push \TT{immedD} into the stack \TT{immedB} times, then add
      \TT{immedC} \\
    & (which must \TT{!= 0}) to the \TT{pc} (skipping \TT{immedC} instructions).
\\\hline
\TT{JMPEQ/JMPNE}
...
\\\hline
\TT{JMPLT/JMPLEQ}
    & Ditto, but \TT{JMP} if the first popped is \\
...
\TT{BEG}/\TT{NOP}/\TT{END}
    & Subtract \TT{immedA} to \TT{sp} (popping the stack \TT{immedA} times). \\
    & For \TT{BEG}/\TT{END} the values popped are always trace values.
\\\hline
\TT{BEGL}
    & Subtract \TT{immedA} to \TT{sp} (popping the stack \TT{immedA} times).
      Then \\
...
variable name is `\TT{*}', discards the value popped.
...
\ttkey{TRACE\_LINES}, \TT{TRACE\_NOJMP}).  This is implemented
