...
\verb|          min::MISSING_PHRASE_POSITION )|
\\[0.5ex]
\verb|void mexstack:| \MEXSTACKKEY{cont} \label{CONT} \\
\verb|    ( mex::instr & instr,| \\
\verb|      min::uns32 tvars,| \\
\verb|      min::gen trace_info = min::MISSING(),| \\
\verb|      const min::phrase_position & pp =| \\
...
to indices.
 ...
 combine these tables, so
 variables and functions are all in one table. However for varia
...
Figure~\itemref{ARITHMETIC-INSTRUCTIONS}, except {\tt POWI},
and the run-time
...
most recently assembled module first.
\TT{PUSH} assembles
...
this instruction adds an element to the pending jump list
(by calling
{\tt mexstack::push\_\EOL jmp\_instr}\pagnote{PUSH-JMP-INSTR}).
\end{indpar}
...
At run-time this instruction's actions are as described in
Figure~\itemref{HOUSEKEEPING-INSTRUCTIONS}.  At assembly-time,
\TT{BEG} increments the depth and sets {\tt stack\_limit}\pagnote{STACK-LIMIT}
(by calling {\tt mexstack::begx}\pagnote{BEGX}).
\end{indpar}
...
same lexical level and depth as the \TT{END} instruction, resets
{\tt stack\_limit} to its value when the companion {\tt BEG} was encountered,
decrements the depth, and updates the pending jmp list
(all by calling {\tt mexstack::endx}\pagnote{ENDX}).
\end{indpar}
