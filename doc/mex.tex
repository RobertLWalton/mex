...
instructions are like those {\em trace-info} instructions with
a difference.
...
\hspace*{2em}
\TT{printer}, \TT{optimze}, \TT{trace\_flags},
\TT{excepts}
\end{itemize}
...
\verb|mex::|\MEXKEY{EXCEPTS\_ERROR}
    & \raggedright program terminated successfully (as per
      \TT{MODULE\_END} or \TT{CALL\_END}) but with an excepts bit on in
      both \TT{p->excepts} and \TT{p->excepts\_accumulator}
    & returns \TT{false} \\
\end{tabular}
\end{center}
...
Notes:
\begin{enumerate}
\item If \TT{run\_process} returns \TT{false}, it also prints
an error message.
...
\emkey{mexas-operation} ::=
    ...
    \ttkey{-palines}			& [ditto and include source lines] \\
    \ttkey{-paoff}			& [do not trace
...
{\em mexas-filname} & Name of file, usually ending in \TT{.mex}, containing
		      ...
		      module name.
\\[0.5ex]
\TT{-pa\ldots}	    & Determines whether an instruction is printed after
...
\\[0.5ex]
\TT{-tc\ldots}	    & Determines the trace class of an assembled instruction. \\
		    & \TT{-tcnormal} sets the trace class of the op code
...
\TT{-ex:}{\em ex-list}
		    & Sets the process \TT{excepts} at the start of mex
		      program execution.
\\[0.5ex]
\TT{-o\ldots}       & Sets the process \TT{optimize} flag \TT{on} or \TT{off}
...
\TT{-r} {\em function-name}
		      ...
		      in the modules, most recently assembled first.
\\[0.5ex]
...
enabled.  All code lines not printed for previous instructions
are printed.
