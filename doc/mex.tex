...
\item The depth when {\tt jmp\_target} is called is equal to or less
than $MD$.
...
depth $MD$ not greater than the current depth, and the
given {\tt target}.
Resolved jumps transfer to the current location: \\
\hspace*{3em}\verb|mexcom::output_module->length|

Resolved pending jumps are removed from the list
...
pending jumps are sorted by lexical level in the pending jump list.
\end{indpar}
