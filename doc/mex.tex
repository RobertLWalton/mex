...
If there are not functions, this mirror is exact with the value of
...
If the program and code have no functions, then if
...
The location of a variable at compile time is the value of
{\tt mexstack::run\_stack\_length - 1} (top of stack)
just after the {\tt PUSH...}
instruction that creates the variable is compiled, or just after
the expression that creates the variable's value in the run-time
stack is compiled.
...
the expression that creates the variable's value executes.
...
{\em Var-stack-length} equals {\tt run\_stack\_length}\pagnote{VAR-STACK-LENGTH}
...
these instructions do not update {\tt run\_stack\_length}.
...
arguments.  For this to work, {\tt mex::run\_stack\_length}
...
{\tt mexstack::run\_stack\_length}\pagnote{VAR-STACK-LENGTH}
...
{\tt mexstack::run\_stack\_length}.  Return the number of elements
...
and {\tt mexstack::run\_stack\_length} have been decremented
...
{\tt < mexstack::run\_stack\_limit}\pagnote{RUN-STACK-LIMIT}).
Note that the
...
{\tt run\_stack\_limit} to its value when the companion
