...
\TT{POPS}/\TT{POP} {\em variable-name}
\begin{indpar}
At run-time, pops the process stack and stores the value popped
into the variable names in the variable stack.
At assembly time, pops the variable stack.

The variable stack search for {\em variable-name} is restricted to
elements with the same lexical level as the \TT{POPS}/\TT{POP} instruction.

Note: \TT{POPL}, \TT{POPG}, and \TT{POPA} do not exist as you cannot write
      variables at a different lexical level or in a different module.
\end{indpar}
...
\TT{BEGF}\label{BEGF} {\em function-name} {\em variable-name}\STAR{}
\begin{indpar}
At run-time this instruction's actions are as described in
...
\TT{CALLM} {\em function-name} {\em nargs} {\em variable-name}\STAR{}
\begin{indpar}
At run-time this instruction's actions are as described in
Figure~\itemref{FUNCTION-INSTRUCTIONS}.  At assembly-time,
...
\TT{CALLG} {\em module-name} {\em function-name}
