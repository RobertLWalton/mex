...
functions and ends of loop iterations.

MEX assumes that variables in the stack are read-only, unless
they are being assigned values by a block of code.  Blocks
push the variables they are assigning into the stack just before
the block executes, and then treats these as write-only until
the block ends, at which time they become read-only.

When a function executes, it begins with just its arguments
in the stack.  When it returns, it pops these are everything
...
module execution) is a parameter if the current instruction known
...
pushed by the code being skipped.  Backward skips are not allowed.
