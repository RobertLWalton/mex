...
and variable values are added to the end of
the module's global stack.  The compiler should add
...
\verb|min::uns32 | & \TTMKEY{trace\_depth}{in {\tt mex::ret}} \\
\verb|}                   | \\
\end{tabular}
\\[1ex]
...
It is an error if an object instruction tries to modify
a public object.
...
The locations in the current function frame are in the range
\TT{[ fp[level], length )}.
A location addressed as \TT{sp[- immedX - 1]} must be in the
current function frame.  Stack values popped or moved must be
in the current function frame.
...
\ttkey{RET}
    & Check that \TT{immedB} equals \TT{p->level}.  Then pop element $e$
      from \\
    & the return stack and set \TT{R = $e$.nresults} unless that
          equals \\
    & \TT{mex::ALL\_RESULTS}, in which case set \TT{R = immedC}. Check that \\
    & \TT{immedC >= R}. Then pop \TT{immedC} values from the stack and save \\
    & them.  Set \TT{sp} to \TT{ap[immedB]} (popping the current function \\
    & execution stack values).  Then push into the stack the \TT{R} saved
      values \\
    & last popped from the stack (value last popped is first pushed).  Lastly \\
    & restore the process members saved in \TT{$e$.saved\_\{pc,level,fp,ap\}},
      \\
...
\\\hline
\ttkey{GET}
    & Pop one value from the stack and use it as an attribute label X to read \\
    & an attribute of the object pointed to by \TT{sp[- immedA - 1]}. Then \\
    & push the attribute value read into the stack.  If X is an integer,
      read the \\
    & X+1'st element of the vector of the object, while if X is a string or \\
    & MIN label, read the labelled attribute of the object.  If the value to
      be \\
    & read does not exist, push \TT{min::NONE()} into the stack instead.
\\\hline
\ttkey{GETI}
...
\\\hline
\ttkey{SET}
    & Pop two values from the stack and write the first popped to the
      attribute \\
    & labelled by the second popped of the object pointed to by
      \TT{sp[- immedA - 1]}. \\
    & If the attribute label X is an integer, write the X+1'st element
      of the vector \\
    & of the object, while if X is a string or MIN label, write to the 
      labelled \\
    & attribute in the object.  If X is an integer, the vector element is 
      required \\
    & to exist; otherwise if the attribute does not have a previous value, \\
...
\\\hline
\ttkey{COPY}
    & Replace the top value in the stack, which must be an object,
      by a copy \\
    & of this object.  The copy is writable even if the object copied
      was public.
\\\hline
\ttkey{COPYI}
    & Push a writable copy of \TT{immedD}, which must be an object,
      into the stack.
\\\hline
\end{tabular}
\end{center}
...
\end{tabular} & \TT{JMP\ldots} instructions in
		...
		to \TT{JMPF}.
\\\hline
\ttmkey{SET}{trace class} & The \TT{PUSHOBJ}, \TT{COPY}, \TT{COPYI},
                            \TT{VPUSH}, \TT{SET},
...
There are three exceptions with different {\em from-variables}
...
\\\hline
\TT{PUSHI}     & `{\em from-variable} \TT{=}' is omitted \\
...
\\[1ex]
\TT{COPY}/\TT{COPYI} &
{\em trace-header}\TT{:} {\em to-variable} \TT{<=} {\em value}
\\[1ex]
\TT{VPUSH} &
...
But for {\tt PUSHOBJ}, {\tt COPY}, and {\tt COPYI},
...
\\[0.5ex]
		   & Note that if the \TT{CALL} \TT{immedC} value, the
		   number of results to be returned, equals
		   \TT{mex::ALL\_RESULTS}, the corresponding \TT{trace\_info}
		   should be \TT{min::MISSING}, or should have \underline{no}
		   {\em trace-variables}.
...
\begin{center}
\begin{tabular}{l@{~~~~}l}
\TT{BEG} ~ \TT{BEGL} & Increment by 1. \\
\TT{CALL}+\TT{BEGF} & Save in return stack entry and then increment by 1. \\
\TT{END} & Decrement by 1. \\
\TT{ENDF} ~ \TT{RET} & Restore from return stack entry. \\
\TT{JMP\ldots} ~ \TT{ENDL} ~ \TT{CONT} & Decrement by the instruction's \\
...
An empty \TT{trace\_info} vector is created.
If \TT{f} is not \TT{min::NULL\_STUB}, an empty
\TT{position} vector is created and the module \TT{name} is set
to \TT{f->file\_name}.  Otherwise the \TT{position} vector is set to
\TT{min::NULL\_STUB} and the module \TT{name} is not set.
The \TT{globals} member is set to \TT{min::NULL\_STUB} and the
\TT{interface} member is set to \TT{min::MISSING()}.

The \TT{max\_length} of the two or three empty vectors is initially set to:
\begin{indpar}
\verb|min::uns32 mex::|\MEXKEY{module\_length}\verb|    // Default == 1 << 12.|
\end{indpar}

Note that \TT{mex} works with a module that has no \TT{position} vector,
but \TT{mexcom} and \TT{mexstack} do \underline{not} work with modules
that have no \TT{position} vector.
...
\verb|min::locatable_var<min::printer> mexcom::|
	& \MEXCOMKEY{printer} \\
...
Except for arguments and results of functions,
...
at run time.  The lexical level of an argument is the lexical level
of its function, and the lexical level of a returned value is the
lexical level of the call instruction that called the function.
...
calling code to the function by the \TT{CALL\ldots} instruction and
given the lexical level of the \TT{BEGF} instruction.

Function return valued variables are pushed into
the compiler symbol table by a \TT{CALL\ldots} instruction
(though often the return values are not given variable names).
During run-time, return values are transfered from the
function frame to the caller's stack by the \TT{RET} instruction and
given the lexical level of the \TT{CALL\ldots} instruction.
...
started \underline{incomplete} execution of its containing function.
...
At run-time values of variables in the most recently started incomplete
function execution of lexical level $L$ have locations in
the range: \\
...
The {\tt mexstack::endx} function always calls this
{\tt mexstack::pop\_stacks} function.
...
\verb|bool mexstack::| & \MEXSTACKKEY{restore}
	{\tt ( \begin{tabular}[t]{@{}l}
	       mexstack::compile\_save\_area \& area, \\
	       bool force = false )
	       \end{tabular} }
\end{tabular}
\begin{indpar}
These functions can be used when a module is compiled incrementally
to save and restore the compilation state
and allow the compilation to back up over an increment that had
a compile error or run-time error.
The {\tt save} function is called before compiling
...
incremented while compiling the increment and if {\tt force}
is {\tt false}.

However, these functions do not save and restore the compiler
symbol table size.  This must be done by the compiler, as only
it knows how the symbol table is constructed.  These functions
also do not save and restore the run-time stack length.
...
For \TT{BEG\ldots}, \TT{CONT}, and \TT{END\ldots} instructions
the compiler can alternatively do the following:
...
\begin{center}
\begin{tabular}{l@{~~~~~}l}
{\tt mexstack::begx}\pagnote{BEGX} & for \TT{BEG\ldots} instructions \\
{\tt mexstack::cont}\pagnote{CONT} & for \TT{CONT} instructions \\
{\tt mexstack::endx}\pagnote{ENDX} & for \TT{END\ldots} instructions \\
\end{tabular}
\end{center}
...
the value of the variable referenced by the stack entry
...
\hspace*{3em}({\tt stack\_offset} corrects {\tt SP} when pushing trace values
							      because \\
\hspace*{3em}~trace values are not recorded in the compiler virtual stack)
\end{indpar}
...
compiler virtual stack.  The number of these trace values becomes
...
\item Saved value of {\tt stack\_limit}
      (equals {\tt mexstack::stack\_length + nvars}).
\item The {\tt nvars} argument (set to 0 for \TT{BEG})
