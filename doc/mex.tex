...
is set to \TT{mexstack::stack\_length}, \TT{mexstack::sp[$L$]} is set
...
(its \TT{immedA}) expecting \TT{nresults} return values is compiled,
any symbol table entries referring to the \TT{nargs} arguments
are popped from the stack, and \TT{nargs} is substracted from
\TT{mexstack::stack\_length}.  Then a result variable is pushed into the
symbol table for every return value, and \TT{mexstack::stack\_length}
...
{\em Var-stack-length} equals {\tt stack\_length}\pagnote{STACK-LENGTH}
...
these instructions do not update {\tt stack\_length}.
...
arguments.  For this to work, {\tt mex::stack\_length}
...
If for {\tt K<L},~~~{\tt fp[$K$] <= index < ap[$K$+1]}: \\
\hspace*{3em}\verb|op_code = mex::PUSHL, immedA = index - fp[K], immedB = K|

If for {\tt K<L},~~~{\tt ap[$K$] <= index < fp[$K$]}: \\
...
{\tt < mexstack::stack\_limit}\pagnote{STACK-LIMIT}).
