...
elemets.  The end of this vector is referred to as the
...
\begin{indpar}
{\bf Lemma 1}~~~An execution of a \TT{CALL\ldots} instruction of
...
lexically containing the \TT{CALL\ldots} instruction, and these
cannot have a level greater than $L+1$, or functions at global
level, which must have level \TT{1}.  QED
\end{indpar}

Let $E$ be an incomplete program execution, consisting of the
sequence of instruction executions in the incomplete program execution.
An execution of a function $F$ within $E$ is defined as all the
instruction executions from the first after $F$ is called till the
last when $F$ returns.  A function execution can be complete, meaning
...
$L_1$ and $L_2$.  Let $E_2$ immediately following $E_1$ in the size
...
Then there must be an incomplete function execution $E'$ in the $E$
...
\begin{indpar}
{\bf Lemma 2}~~~Let $E_1$ be an incomplete execution in $E$ of a function $F_1$
of level $L_1$, and let all smaller incomplete function executions in $E$
have level $>L_1$.  Then all smaller incomplete function executions are
executions of functions that are lexically inside $F_1$.
\end{indpar}
\begin{indpar}
{\bf Proof}: We step inductively through the sequence of incomplete
function executions of $E$ from $E_1$ to the end of the sequence.

The first incomplete function execution after $E_1$ in the sequence
must by Corollary 1 of Lemma 1 have level $\leq L_1+1$, but by hypothesis
it has level $>L_1$, it must have level $L_1+1$.  It is called by a
\TT{CALL\ldots} instruction in $E_1$ that has level $L_1$ and therefore
can only reference functions of level $L_1+1$ that are lexically inside
$F_1$.

Now let $E_2$ be an incomplete function execution inside $E_1$
(later in the squence) that is an incomplete execution of a function
$F_2$ lexically
inside $F_1$, and consider the next incomplete function execution.
This called by a \TT{CALL\ldots} instruction inside $F_2$ of level
$>L_1$, and 
...
\TT{PUSHL} with \TT{immedB} = $L'$ (Figure~\ref{HOUSEKEEPING-INSTRUCTIONS}),
...
The \ttikey{name}{of module} member of the \TT{module} is derived
...
from standard input).  This information can be used to
...
Trace-Info Based Trace Messages,
\itemref{TRACE-INFO-BASED-TRACE-MESSAGES}.
...
\hline
\TT{SET\_TRACE}
    & The process \TT{trace\_flags} member is set to \TT{immedA}. \\
...
\\\hline
\TT{SET\_EXCEPTS}
    & The process \TT{excepts} member is set to \TT{immedA}.
\\\hline
\end{tabular}
\end{center}
\vspace*{-3ex}
...
may raise the exceptions in Figure~\ref{EXCEPTIONS}.
\begin{boxedfigure}[b!]
\begin{center}
\begin{tabular}{|l|l|}
\hline
...
a process's \TT{excepts} member is:
\begin{center}
...
In C/C++ the \TT{excepts} bit associated with except \TT{XX}
is \TT{FE\_XX}, as defined as a bit mask macro in \TT{fenv.h}
...
Instructions in the \ttkey{ALWAYS} trace class are always
...
(see The MEXAS Program, \itemref{THE-MEXAS-PROGRAM}).
...
to set the trace flags of a process to a pre-defined group of trace classes.
Trace class names and trace group names are used by compiler switches
which set the initial trace flags of processes, and by the
\TT{SET\_EOL TRACE} instruction that sets the current trace flags
of a process.

Notice that all arithmetic instruction have the same
...
\ttikey{PUSH}{trace class} & All \TT{PUSH\ldots} instructions in Figures
    \ref{STACK-AND-FLOW-CONTROL-INSTRUCTIONS} and
    \ref{FUNCTION-INSTRUCTIONS}.
\\\hline
\ttikey{POP}{trace class} & The \TT{POP} instruction in Figure
    \ref{STACK-AND-FLOW-CONTROL-INSTRUCTIONS}.
\\\hline
...
or equals \TT{min::MISSING()}, only the message beginning just
given is produced.
...
in Figure~\ref{STACK-AND-FLOW-CONTROL-INSTRUCTIONS}
the value of
...
{\em trace-info}.  Its form is:
\begin{indpar}
{\em trace-info-message} ::=
    {\em trace-message} ~
...
For trace-info based instructions
the value of
\TT{module->\EOL trace\_\EOL info[i]} is the MIN label:
\begin{center}
\vspace*{-1ex}
\TT{[<} {\em trace-message} ~ {\em trace-variable}\STAR{} \TT{>]}
\end{center}

\subsubsection{\TT{BEGL}, \TT{ENDL}, and \TT{CONT} Trace Messages}
\label{BEGL-ENDL-AND-CONT-TRACE-MESSAGES}

The traces of the \TT{BEGL}, \TT{ENDL}, and \TT{CONT}
instructions are like those of {\em trace-info} instructions with
a difference.

The {\em trace-message} for all these instructions is taken
from the loop \TT{BEGL} instruction, and the {\em trace-variables}
are the {\em nnext} loop variables in the variables stack
when the \TT{BEGL} instruction is compiled.  When traced,
\TT{BEGL} reads the values of these from the execution stack
when the \TT{BEGL} is encountered.  When traced,
\TT{CONT} and \TT{ENDL} first pop the execution stack so it
holds only loop variables and \TT{next-\ldots} variables,
then copies the \TT{next-\ldots} variables into the loop variables,
and then reads the top {\em nnext} variable values and uses them
in the trace.  These are \TT{next-\ldots} variable values, but
at the time they are traced they equal the loop variable values,
and so the loop variable values at the start of each iteration
are traced, and are given the loop variable names and \underline{not}
\TT{next-\ldots} names.

\subsubsection{\TT{CALL}, \TT{RET}, and \TT{ENDF} Trace Messages}
\label{CALL-RET-AND-ENDF-TRACE-MESSAGES}

The traces of the \TT{CALL\ldots}, \TT{RET}, and \TT{ENDF}
instructions are like those of {\em trace-info} instructions with
a difference.

For \TT{CALL\ldots}, the op code in the trace is replaced by
`\TT{CALL\ldots to BEGF}', the {\em trace-message} is taken
from the target \TT{BEGF} (this defaults to the function name),
and the {\em trace-variables} are the argument variables listed
in the \TT{BEGF} instruction.  The most important thing to note
is that the argument variable names are those used inside the
function, and not those used at the point of the call instruction.

For \TT{RET}, the op code in the trace is replaced by
`\TT{RET to CALL\ldots}', the {\em trace-message} is taken
from the target \TT{CALL\ldots} (this defaults to the function name),
and the {\em trace-variables} are the result variables listed
in the \TT{CALL\ldots} instruction.  The most important thing to
note is that the result variable names are those used after the
call instruction and not those used inside the function
at the point of the return instruction.

\TT{ENDF} is just like a \TT{RET} with no return values.
...
\\\hline
\TT{SET\_TRACE} & \TT{SET\_TRACE:} {\em trace-class}\STAR{} \\
                & The {\em trace-classes} being enabled are
	          listed, but they are \underline{not}
		  aggregated into {\em trace-class-groups}.
\\[0.5ex]
\TT{SET\_EXCEPTS} & \TT{SET\_EXCEPTS:}
                    \{ \TT{*}\QMARK{}{\em except-name} \}\STAR{} \\
                & The names of the excepts being enable in
		  \TT{p->excepts} are listed, and those that are
		  also in \TT{p->excepts\_accumulator} are starred with
		  \TT{*}.
\\[0.5ex]
\TT{TRACE\_EXCEPTS} & \TT{TRACE\_EXCEPTS:}
                    \{ \TT{*}\QMARK{}{\em except-name} \}\STAR{} \\
                & The names of the excepts in \TT{p->excepts\_accumuator}
		  are listed, and those that are also in \TT{p->excepts}
		  are starred with \TT{*}.
\end{tabular}
\end{center}
...
the trace depth).  Housekeeping instructions (those in
Figure~\ref{HOUSEKEEPING-INSTRUCTIONS}) execute in the same manner regardless
of the setting of \TT{optimize}, and can be traced when \TT{optimize}
is \TT{true}.  In particular, the \TT{TRACE} instrution works regardless
of the setting of \TT{optimize}.
...
The \TT{trace\_flags}, \TT{excepts}, and \TT{optimize} members
...
\\[0.5ex]
\verb|int mex::| & \MEXKEY{run\_excepts} \\
...
\TT{counter}, and \TT{excepts\_\EOL accumulator}
...
The \TT{fp[\ldots]}, \TT{nargs[\ldots]}, \TT{level}, \TT{trace\_\EOL depth},
\TT{counter}, and \TT{excepts\_\EOL accumula\-tor}
members are set to \TT{0}.
The \TT{state} is set to \TT{NEVER\_\EOL STARTED}.

The stack sizes, \TT{counter\_limit}, \TT{trace\_flags},
\TT{excepts}, and \TT{optimize} members are
...
\verb|mex::|\MEXKEY{EXCEPTS\_ERROR}
    ...
    & but with an excepts bit on in both \\
    & \TT{p->excepts} and \TT{p->excepts\_accumulator} \\
\end{tabular}
\end{center}

Notes:
\begin{enumerate}
\item On \TT{\ldots\_LIMIT\_STOP}, you can increase the limit
and call \TT{run\_process} again to continue execution.
For stacks you must resize the stack.
...
than whitespace, and non-ASCII characters, are illegal and are
replaced by `\#'.
...
illegal characters).  Blank lines are ignored.

If the last lexeme on a line is `\ttkey{\textbackslash}', the statement
...
the element just gets the name `\TT{*}'.
...
\\[0.5ex]
\emkey{trace-info} ::= {\em trace-message} {\em trace-variable}\STAR{}
\end{indpar}
...
with the given {\em new-variable-name}.
\end{indpar}
...
At assembly-time, this instruction changes the name of the top
variable stack element to {\em new-variable-name}.
\end{indpar}
...
\ttkey{NOP}/\ttkey{TRACE}/\ttkey{WARN}/\ttkey{ERROR} {\em trace-info}\QMARK{}
\begin{indpar}
...
\begin{indpar}
At run-time set the process \TT{excepts} parameter so as
...
\emkey{mexas-operation} ::=
    ...
    \ttkey{-ex:}{\em ex-list}		& [run with listed excepts] \\
    \ttkey{-oon}			& [run with optimize on] \\
    \ttkey{-ooff}			& [run with optimize off] \\
...
{\em mexas-operations} until it is overriden.  E.g.,
\TT{-oon} will cause following all executions to be optimized
until an \TT{-ooff} is encountered.
...
\TT{-tcnormal}	    & \TT{-tcnormal} sets the trace class of the op code
		      given by Figure~\ref{TRACE-CLASSES-AND-GROUPS}.
...
\\[0.5ex]
\TT{-ex:}{\em ex-list}
		    & Sets the process \TT{excepts} at the start of mex
		      program execution.
...
\end{indpar}
The {\em line-number} is the number of the \underline{last} source code
line containing code that generated the instruction (the first line is
