...
\ttkey{JMPTRUE}
    ...
    & If \TT{immedB} = 0, continue as per \TT{JMP} if no, but otherwise \\
    & do nothing more.  Else if \TT{immedB} > 0, continue as per \\
    & \TT{JMP} if yes, but otherwise do nothing more.
\\\hline
\ttkey{JMPCNT}
...
\ttkey{JMPTRUE} ~ \TT{*}\QMARK{} {\em jmp-target}
\begin{indpar}
At run-time this instruction's actions are as described in
Figure~\itemref{STACK-AND-FLOW-CONTROL-INSTRUCTIONS}, where the \TT{*}
option sets \TT{immedB} = 1 (otherwise it is 0).  At assembly-time,
