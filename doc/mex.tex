...
\\[1ex]
\verb|const unsigned mex::|\MEXKEY{max\_lexical\_depth}\verb| = 16|%
...
\itemref{NESTED-FUNCTIONS}.  Note that \TT{fp[0]}, the frame pointer
...
The \TT{sp}, \TT{rp}, \TT{fp[\ldots]}, \TT{trace\_depth},
...
or declarations, each encoded as a sequence of lexemes on
a line.  A line ending with the `\ttkey{\textbackslash}'
is \key{continued} on the next line (the `\textbackslash' is
otherwise ignored).
If the first lexeme on a line is `\ttkey{//}', the entire line
is a \key{comment line} and is ignored.  Blank lines are ignored.

Instructions begin with an op code, and the program syntax
of instructions is that of a code vector: see Code Vectors,
\itemref{CODE-VECTORS}.

Declarations associate names with stack locations and code
locations.
...
from a process stack are \TT{CALL} and \TT{JMP}.  \TT{CALL}
...
can be called by a \TT{CALL} instruction.  \TT{BEGF} pushs
...
and those that are found are processed and removed from
the list.  Processing involves calculating the distance
between the jump and its target, and the number of
process stack elements the jump should pop and push.

To do this, an element of the pending jump list contains:
\begin{enumerate}
...
\item The length $S$ of the variable stack when the jump instruction
      was encountered.
\item The minimum $M$ of all the variable stack lengths that have
      occured since the jump instruction was encountered.
      $M\le S$.
\end{enumerate}

A \TT{JMP\ldots} instruction for which a legal target exists
is called \key{satisfied}.  Other \TT{JMP\ldots} instructions
are \key{unsatisfied} and are in error.

The following instructions/declarations act on the pending jump list:

\TT{JMP\dots} {\em jmp-target}
\begin{indpar}
...
depth, satisfy these, and remove them from
...
lexical level of the \TT{ENDF} and remove them as being unsatisfied,
...
element, `\TT{*}' can be used as the {\em variable-name} to indicate
that the element should have no name.
\item The operands that take integer values must have values in the
range $[0,2^{16})$ (so they will fit into \TT{immed\ldots} instruction
members.
\end{enumerate}

{\em aop} {\em variable-name}\QMARK{}
\begin{indpar}
...
\TT{PUSHS} {\em variable-name}
          {\em new-variable-name}\QMARK{}
\begin{indpar}
At run-time, pushes into the process stack a copy of the variable
named in the variable stack.
At assembly time, pushes into the variable stack an
element whose name is the {\em new-variable-name} if that is
given, or {\em variable-name} otherwise.

The variable stack search for {\em variable-name} is restricted to
elements with the same lexical level as the \TT{PUSH} instruction.
\end{indpar}

\TT{PUSHL} {\em variable-name}
          {\em new-variable-name}\QMARK{}
\begin{indpar}
At run-time, pushes into the process stack a copy of the variable
named in the variable stack.
...
The variable stack search for {\em variable-name} is \underline{not}
restricted as to the lexical level of the elements searched.
\end{indpar}

\TT{PUSHM} {\em variable-name}
           {\em new-variable-name}\QMARK{}
\begin{indpar}
...
the {\em variable-name} otherwise.

The variable stack search for {\em variable-name} is restricted to
elements with the lexical level \TT{0}.
\end{indpar}

\TT{PUSHI} {\em number} {\em variable-name}\QMARK{}
\begin{indpar}
...
with the given {\em variable-name}, if provided.
\end{indpar}

\TT{PUSHG} {\em module-name} {\em variable-name}
           {\em new-variable-name}\QMARK{}
\begin{indpar}
At run-time, pushes into the process stack a copy of the named
...
stack and proceeds through the global variables of other modules
in the order that the modules were compiled.
\end{indpar}

\TT{POPS} {\em variable-name}
\begin{indpar}
...
\TT{POPM} {\em variable-name}
\begin{indpar}
At run-time, pops the process stack and stores the value popped
in the named lexical level 0 variable.
At assembly time, pops the variable stack.

The variable stack search for {\em variable-name} is restricted to
elements with lexical level \TT{0}.
\end{indpar}

\TT{POP} {\em variable-name}
\begin{indpar}
At run-time, pops the process stack and stores the value popped
...
be a \TT{LABEL} declaration, a \TT{BEG\ldots} instruction,
or an \TT{END\ldots} instruction.
\end{indpar}

\TT{JMP\ldots} {\em jmp-target}
\begin{indpar}
...
and then add an element to the pending jump list.  Note that
...
At assembly-time, this instruction pushes {\em nnext} variable
...
At run-time these instruction's actions are as described in
Figure~\itemref{HOUSEKEEPING-INSTRUCTIONS}.  At assembly-time,
they pop from the variable and function stacks all elements with the
same lexical level and depth as the \TT{END}/\TT{ENDL}
instruction.
\end{indpar}

\TT{SET-TRACE} {\em trace-option}\STAR{}
\begin{indpar}
At run-time set the process \TT{trace\_flags} parameter to the
flags given by the options (i.e., \TT{TRACE},
\TT{TRACE\_PHRASE}, \TT{TRACE\_NOJMP}).  At assembly-time,
this instruction does nothing.
\end{indpar}

\TT{ERROR} {\em error-code} {\em nargs}\QMARK{}
\begin{indpar}
If {\em error-code} is non-zero, this instruction terminates
the process at run-time, {\em nargs} must \underline{not}
be given, and this instruction does nothing at assembly-time.

If {\em error-code} is \TT{0}, {\em nargs} must be given
and this instruction pops {\em nargs} elements from the
process stack at run-time and from the variable stack
at assembly-time.
\end{indpar}

\TT{BEGF} {\em function-name} {\em variable-name}\STAR{}
\begin{indpar}
At run-time this instruction's actions are as described in
...
It is \underline{not} necessary that the number of
{\em variable-names} exactly equals the number of actual
arguments.
\end{indpar}
...
same lexical level \TT{ENDF} instruction.
\end{indpar}

\TT{CALL} {\em function-name} {\em nargs} {\em variable-name}\STAR{}
\begin{indpar}
At run-time this instruction's actions are as described in
Figure~\itemref{FUNCTION-INSTRUCTIONS}.  At assembly-time,
