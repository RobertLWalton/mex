...
code vector.  The {\tt no\_source} and {\tt stack\_offset} parameters
...
\item The minimum $M$ of $S$ and all the variable stack lengths immediately
...
is such that:
\begin{enumerate}
\item The lexical level when {\tt jmp\_target} is called equals $L$.
Equivalently, there is no {\tt ENDF} instruction between the
{\tt JMP\ldots}~and the {\tt jmp\_target}.
\item The depth when {\tt jmp\_target} is called equals $K$.
Equivalently, the call to {\tt jmp\_target} is not inside a block whose
{\tt BEG/BEGL} instruction is between the {\tt JMP\ldots}~and
the {\tt jmp\_target}.
\item The value of {\tt var\_stack\_length} when {\tt jmp\_target}
is called \underline{equals} $M$.
\end{enumerate}
...
The following MEXAS assembler program illustrates this
rule when the jump does \underline{not}
reduce depth and $M$ = the variables stack length just after the
\TT{JMP\ldots} is compiled (in MEXAS the {\tt LABEL} statements
call {\tt jmp\_target}):
...
The following are the {\tt mexstack} component {\tt JMP}
...
and {\tt instr.trace\_class} must be set.
This function executes {\tt mexstack::push\_instr}
to push that instruction into the {\tt mexcom::\EOL output\_\EOL module}
code vector.  The {\tt no\_source} and {\tt stack\_offset}
