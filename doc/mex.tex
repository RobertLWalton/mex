...
Proof of Theorem: TBD

Let $F'$ be the unique function of level $L'$ that
lexically contains $F$.  Then all \TT{CALL} statements that invoke an execution
of $F$ must be lexically inside $F'$, and therefore inside an execution of $F'$,
so every execution of $F$ is inside an execution of $F'$.
Conversely, if $F'$ is a function that does \underline{not} lexically contain
$F$, any execution of $F'$ cannot contain a \TT{CALL} to $F$ and therefore
cannot contain an execution of $F$.
