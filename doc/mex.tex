...
A \key{number} is a lexeme acceptable as an IEEE floating point
number to the POSIX \TT{strtod} function.

A \key{name} is a lexeme beginning with a letter that is
...
\begin{enumerate}
\item Unless otherwise specified, when an instruction pushes an
...
the top of the stack hides the element lower down.
\item An \emkey{aop} is an arithmetic intruction operation code,
as in Figure~\itemref{ARITHMETIC-INSTRUCTIONS}.
\item When an instruction pushes an element in an assembler stack,
the element is assigned the lexical level and depth of the instruction.
...
\item The operands {\em immedA}, {\em immedB}, and {\em immedC},
if given, must be integers in the range $[0,2^{16})$.
\end{enumerate}

{\em aop} {\em variable-name}\QMARK{}
\begin{indpar}
The same number of elements are popped from the variable stack
as would be popped from the process stack during execution of the
arithmetic instruction {\em aop}.
Then one element is pushed.  If {\em variable-name} is given, it become
the name of the pushed element, which otherwise has no name.
\end{indpar}

\TT{PUSH} {\em variable-name}
\begin{indpar}
At run-time, pushes into the process stack a copy of the named
variable.  At assembly time, pushes into the variable stack a
duplicate of the named variable stack element.
...
\begin{indpar}
At run-time, pushes into the process stack a copy of the {\em number}.
...
The search of an element with the given {\em variable-name}
...
Figure~\itemref{HOUSEKEEPING-INSTRUCTIONS}.  At assembly-time,
duplicates of the top {\em immedB} stack entries are 
pushed into the variable stack, but if a stack element being
duplicated has name $N$, then its duplicate is given name
\TT{next-}$N$.
\end{indpar}
...
At run-time these instruction's actions are as described in
Figure~\itemref{HOUSEKEEPING-INSTRUCTIONS}.  At assembly-time,
pop from the variable and function stacks all element with the
...
\TT{ERROR} {\em immedA}\QMARK{}
\begin{indpar}
If {\em immedA} is given, pop {\em immedA} element from the
variable stack at assembly-time and from the process stack
at run-time ({\em immedA} may be \TT{0}).  
If {\em immedA} is \underline{not} given, terminate the process
at run-time and do nothing at assembly-time.
\end{indpar}
