...
\begin{indpar}
{\bf Assumption}~~~If $F$ is a function that is lexically inside another
function $F'$, then \underline{all} \TT{CALL} statements that invoke $F$
must be lexically inside $F'$.

{\bf Theorem}~~~If $F$ is a function of level $L$ and $1\le L'<L$,
and if $E$ is an execution of $F$, then the smallest execution $E'$ of some
function of level $L'$ such that $E\subset E'$ exists and is an execution
...
Justification of Assumption: The assumption will be true of any
compiled language that requires a \TT{CALL} to a level $L>1$ function $F$
to be within the scope of the function definition of $F$, and requires that
scope to be lexically within the the function lexically
containing the definition of $F$.

Proof of Theorem:
The set of current incompleted function executions is a
...
to the functions).  Let $F'$ be the function of level $L'$ that
lexically contains $F$.  Then the \TT{CALL} to $F$ is inside $F'$
but might be inside another function $F_1$ that is inside $F'$
(it is possible for $F_1$ to be $F$ since recursion is allowed).
If $F_1$ is $F'$ we are done.  If not, apply the same reasoning to
$F_1$ to get the function $F_2$ that called it.  If $F_2$ is $F'$
we are done, but if not $F_2$ is inside $F'$ so use recursion to continue.
It is impossible
to get to the beginning of the return stack with only functions that
are inside $F'$, as then the first function in the return stack
would be of level $>L'\ge1$ which is impossible (it must be of level 1).

Proof of Corollary:  By inspection of the actions of the \TT{CALL}
and \TT{RET} statements of Figure~\ref{FUNCTION-INSTRUCTIONS},
the frame pointer \TT{fp[$L'$]} is the frame pointer
of the \underline{last} \TT{CALL}ed incomplete execution
...
accessed by \TT{PUSHA} with \TT{immedB} = $L'$, and
global variables can be accessed by \TT{PUSHM} or \TT{PUSHG}:
see Figures~\ref{FUNCTION-INSTRUCTIONS}%
~and~\ref{HOUSEKEEPING-INSTRUCTIONS}.
...
\TT{PUSH}
    ...
    & into the stack (i.e., \TT{sp[- immedA]}, so \TT{immedA} = 0 duplicates \\
    & the top value).
\\\hline
\TT{PUSHI}
...
\TT{PUSHG}
    ...
    & (i.e., \TT{m[immedA]} where \TT{m} is the module)
\\\hline
\TT{POP}
    & Pop 1 value and store it in the \TT{immedA}'th value in the stack below \\
    & the value popped (i.e., store to \TT{sp[- immedA]}, so \TT{immedA} = 0 \\
