...
\ttkey{JMPEQ}/\ttkey{JMPNEQ}
    ...
    & continue as per the \TT{JMP} instruction.  Otherwise if
      \TT{immedB} == 0, \\
    & do nothing more, but if \TT{immedB} == 1, push the first value \\
    & popped back into the stack.
\\\hline
...
as a MIN string or number.
If \TT{module->\EOL trace\_\EOL info[i]} is not a MIN string or number,
