...
\verb|    min::uns32 | & \TTMKEY{trace\_info}{in {\tt mex::ret}} \\
\verb|}                                  | \\
\end{tabular}
...
and the \TT{trace\_info} member is used for tracing
(see \pagref{MEX::TRACE}).
...
value's \TT{saved\_fp} member are explained in \itemref{NESTED-FUNCTIONS}.
...
\TT{BEGF}
    ...
    & target functions lexical nesting level (\pagref{LEXICAL-NESTING-LEVEL}),
                                      and \TT{immedD} is saved \\
    & in the the \TT{met::ret} value's \TT{trace\_info} member.
\\\hline
\TT{ENDF}
    & Add \TT{immedA} to \TT{sp} (popping the stack \TT{immedA} times),
    					then pop \\
...
\TT{CALL}
    ...
    & \TT{immedB} value, set the \TT{mex:ret} value's \TT{saved\_fp} member
      to the \\
    & process's \TT{fp[$L$]} member, set the process's \TT{fp[$L$]}
      member to the \\
    & process's \TT{sp} member, and set the \TT{mex::ret}
      value's \TT{trace\_info} \\
    & member from the \TT{BEGF} instruction's \TT{immedD}.
    \\[0.5ex]
