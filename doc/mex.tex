...
\ttkey{JMPSTR}/\ttkey{JMPOBJ}
    ...
    & the test, so `no' jumps and `yes' does not.  For \TT{JMPTRUE} it is \\
    & a run-time error if the popped value does not equal \\
    & \TT{mex::TRUE} or \TT{mex::FALSE}.
\\\hline
\ttkey{JMPCNT}
...
\ttkey{JMPTRUE}/\ttkey{JMPINT}/\ttkey{JMPFIN}/\ttkey{JMPINF}/\ttkey{JMPNUM}/%
    \ttkey{JMPSTR}/\ttkey{JMPOBJ} ~ \ttkey{*}\QMARK{} {\em jmp-target}
\begin{indpar}
