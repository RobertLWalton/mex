...
the \TT{return\_stack} and stores it in the current program counter
of the process (the \TT{pc} member of the \TT{process}).

The \TT{trace\_flags}, \TT{excepts}, and \TT{optimize}
...
\hline
\TT{PUSH}
    & Push the \TT{immedB}'th value in the stack below the top value \\
    & into the stack (\TT{immedB} = 0 duplicates the top value).
\\\hline
\TT{PUSHI}
    & Push \TT{immedD} into the stack.
\\\hline
\TT{PUSHG}
    & Push into the stack the \TT{immedB}'th value from the end of the \\
...
\TT{ENDF}
    & Add \TT{immedA} to \TT{sp} (popping the stack \TT{immedA} times),
    					then \\
    & pop the return stack and set the \TT{pc} to the value popped.
\\\hline
\TT{CALL}
    & Push the \TT{pc} value in to the return stack, then
                               set the \TT{index} \\
    & of the \TT{pc} to \TT{immedC} (this should be the \\
    & first instruction \underline{after} the \TT{BEGF} of the function.
\\\hline
\TT{CALLG}
    & Push the \TT{pc} value in to the return stack, then
                      set the \TT{index} \\
    & of the \TT{pc} to \TT{immedC} and the \TT{module}
                            of the \TT{pc} to \TT{immedD} \\
    & (which must convert to a \TT{mex::module}).
\\\hline
\TT{RET}
    & Pop \TT{immedB} values from the stack and save them.  Then \\
    & add \TT{immedA} to \TT{sp} (popping the stack \TT{immedA} times), \\
    & then push the saved values into the stack, then pop \\
    & the return stack and set the \TT{pc} to the value popped.
\\\hline
\end{tabular}
\end{center}
\caption{Non-Arithmetic Instructions}
\label{NON-ARITHMETIC-INSTRUCTIONS}
\end{boxedfigure}
...
The non-arithmetic instruction set is in
Figure~\ref{NON-ARITHMETIC-INSTRUCTIONS}.
...
\TT{I} is such that the process's \TT{1ull <{}< I} \TT{trace\_flags}
bit is on, the instruction is traced.  In optimized mode,
no tracing is done.  The default setting of the \TT{trace\_flags}
member of a process is \TT{0}.

When an instruction is traced, the instruction prints its
\TT{min::phrase\_position} (from its module's
\TT{min::phrase\_\EOL position\_vec}) followed by
a description of the value produced by the instruction.

Instructions may raise the following exceptions, each
...
I both modes, if any instruction raised an exception whose bit
