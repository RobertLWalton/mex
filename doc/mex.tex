...
\skey{trace packet variable}s.\label{TRACE-PACKET-VARIABLE}
...
print source lines after error message.  Printing is done using
...
MEX and {\tt mexstack} assume that the compiler
...
When the compiler outputs a {\tt PUSH\ldots}
instruction that creates a variable, it pushes the name of the variable
into the variable stack.  When the compiler outputs a {\tt BEGF}
instruction that creates a function, it pushes the name of the function
into the function stack if the function has arguments, and into the
...
The compiler actually implements the stacks as symbol tables optimized
...
trace packet variables\pagnote{TRACE-PACKET-VARIABLE}) in the process
stack are not recorded in the compiler variables stack.
...
The {\tt mexstack} component also maintains the following:
...
{\tt Lp[$L$]} is the index of the first variable of level $L$ in the
variable stack, or more specifically, for all $i<${\tt lp[$L$]},
the variable with index $i$ has level $<L$.

{\tt Fp[$L$]} is the index of the frame pointer in the variable
stack of the function of level $L$ currently being compiled.  More
specifically, {\tt fp[$L$] = lp[$L$]} plus the minimum number of arguments
...
\end{tabular} \\
Here {\tt var\_stack\_length} is the value before the {\tt BEG\ldots}
...
before the corresponding {\tt BEG\ldots} was compiled.

The {\tt mexstack::init} function (see below) zeros these variables.
\end{indpar}

The following function uses the {\tt mexstack} data described above:
...
with the given {\tt index}, and executes {\tt mex::push\_instr}
to push that instruction into the {\tt mexcom::output\_module}
code vector.
...
If {\tt index >= lp[L]}: \\
\hspace*{3em}\verb|op_code = mex::PUSHS, immedA = SP + sp_offset - 1 - index|
...
\hspace*{3em}({\tt sp\_offset} corrects {\tt SP} when pushing trace values
							      because \\
\hspace*{3em}~trace values a not recorded in the compiler variable stack)
\end{indpar}
...
\begin{indpar}
This function initializes {\tt mexstack} component data at the beginning
...
\verb|void mexstack::| \MEXCOMKEY{trace\_instr} \\
\verb|        ( min::uns32 location,| \\
\verb|          min::uns32 stack_length,| \\
...
{\em Var\_stack\_length}\pagnote{VAR-STACK-LENGTH}
is the value \underline{after}
the instruction is ouput (for {\tt PUSH\ldots} instructions
that push {\em trace-packet-variables}\pagnote{TRACE-PACKET-VARIABLE}
the number of {\em trace-packet-variables} pushed so far is added, as
these instructions do not update the compiler variables stack).
The {\em trace-depth} is printed only if it is not zero,
...
If {\tt mexstack::\EOL trace\_\EOL never} is {\tt true} and
additionally {\tt instr.trace\_class != mex::\EOL T\_ALWAYS}, first modify
...
\verb|void mexstack:| \MEXSTACKKEY{begx} \\
\verb|    ( mex::instr & instr,| \\
...
\verb|          min::MISSING_PHRASE_POSITION )|
\\[0.5ex]
\verb|void mexstack:| \MEXSTACKKEY{endx} \\
\verb|    ( mex::instr & instr,| \\
...
\verb|          min::MISSING_PHRASE_POSITION )|
\\[0.5ex]
\verb|void mexstack:| \MEXSTACKKEY{cont} \\
\verb|    ( mex::instr & instr,| \\
\verb|      min::uns32 tvars,| \\
\verb|      min::gen trace_info = min::MISSING(),| \\
\verb|      const min::phrase_position & pp =| \\
...
its {\tt op\_code} and {\tt trace\_class}.  The compiler must then
compute the instruction's position {\tt pp} and {\tt trace\_info}.
If the {\tt trace\_info} has
trace packet variables\pagnote{TRACE-PACKET-VARIABLE}, the
...
the {\tt tvars} parameter of these functions.

Lastly, in the case of the {\tt BEGL} instruction, the compiler must
...
\subsubsection{MEX Stack Component Jump Management}
\label{MEX-STACK-COMPONENT-JUMP-MANAGEMENT}

TBD

A {\em jmp-target} is a MIN string that identifies
a code vector index value which can be used by \TT{JMP\ldots}
instructions as the point in the code vector to jump to.

\TT{JMP\ldots} instructions pushed into the code vector
...
{\tt mexstack} maintains a list of \skey{pending jumps}:
...
\item The {\em jmp-target} MIN string.
\item The lexical level $L$ of the {\tt JMP\ldots} instruction.
\item The maximum depth $D$ allowed for the target.
\item The length $S$ of the variable stack just after the {\tt JMP\ldots}
      instruction pops variable elements from the stack.
...
is such that:
\begin{enumerate}
\item There is no {\tt ENDF} instruction between the {\tt JMP\ldots}
and the call to {\tt jmp\_target}.
\item The call to {\tt jmp\_target} is not inside a block whose
{\tt BEG/BEGL} instruction is between the {\tt JMP\ldots}
and the call to {\tt jmp\_target}.
\item The value of {\tt var\_stack\_length} when {\tt jmp\_target}
is called \underline{equals} the minimum of its value just after
the {\tt JMP\ldots} instruction pops any operands it has and
the values of {\tt var\_stack\_length} just after any {\tt END/ENDL}
instructions that are between the {\tt JMP\ldots} instruction and the
call to {\tt jmp\_target}.
\end{enumerate}
