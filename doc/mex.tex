...
\hline
\ttkey{NEW\_OBJ}
    & Push a pointer to a new object with \TT{immedA} unused size and \\
    & \TT{immedB} hash size into the stack.
\\\hline
\ttkey{VPUSH}
    ...
    & at the top of the stack itself an object with 
                    \TT{.initializor == immedD}, \\
    & push instead the vector elements of this object.  After finishing \\
...
\ttkey{VPOP}
    ...
    & \TT{sp[- immedA -1]} and then push that value into the stack.  However, \\
    & if the vector object is empty, push \TT{min::NONE()} instead.
\\\hline
\ttkey{GET}
    & Read the attribute named by the value X of \TT{sp[- immedC -1]} from \\
    ...
    & named attribute of the object.  If the value to be read does not exist, \\
...
\\\hline
\ttkey{GETI}
    & Read the attribute named by \TT{immedD}, which must be a string \\
...
\\\hline
\ttkey{SET}
    & Write the top value in the stack to the attribute named by the value \\
    & X of \TT{sp[- immedC -1]} in the object pointed to by
    	\TT{sp[- immedA -1]}. \\
    & If X is an integer, write the X+1'st element of the vector of the \\
    & object, while if X is a string or MIN label, write to the named \\
    ...
    & require \TT{immedC} to equal \TT{1} and pop a second value (X) \\
    & from the stack.
\\\hline
\ttkey{SETI}
    & Write the top value in the stack to the attribute named by \TT{immedD}, \\
...
\end{tabular} & \TT{JMP\ldots} instructions in
		...
		to \TT{JMPF}.
\\\hline
\ttmkey{SET}{trace class} & The \TT{NEW\_OBJ}, \TT{VPUSH}, \TT{SET},
...
For \TT{NEW\_OB},
\TT{module->\EOL trace\_\EOL info[i]} is the {\em to-variable} label.
...
to the bottommost, and accept the first element found with the desired name.
...
\item Some instructions have an {\em attribute-label}\label{ATTRIBUTE-LABEL}
argument which may be a {\em string} or may have the form
\begin{indpar}
\TT{[} {\em string}\PLUS{} \TT{]}
\end{indpar}
This latter form specfies a MIN label whose elements are the {\em string}s.
...
or \TT{mex::TRUE}, or \TT{mex::FALSE}, or an empty MIN object.
At assembly time, pushes into the variable stack an
element with {\em new-variable-name} as its name.
\end{indpar}
...
\ttkey{JMPTRUE}/\ttkey{JMPFALSE}/\ttkey{JMPINT}/\ttkey{JMPFIN}/%
    \ttkey{JMPINF}/\ttkey{JMPNUM}/\ttkey{JMPTRUTH}/%
    \ttkey{JMPSTR}/\ttkey{JMPOBJ} \\
...
\ttkey{NEW\_OBJ} ~ {\em unused-size} ~ {\em hash-size} ~
                   {\em new-variable-name}\QMARK{}
\begin{indpar}
At run-time this instruction's actions are as described in
Figure~\itemref{OBJECT-INSTRUCTIONS}.
At assembly time, pushes into the variable stack an
element with {\em new-variable-name} as its name.
\end{indpar}
...
attribute name value \EOL (\TT{immedC} value).
